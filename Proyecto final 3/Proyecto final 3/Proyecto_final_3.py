...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    compound_stat = list(map(lambda stat: (stat[info["playerid"]],
                                           formula(info, stat)), statistics))
    compound_stat.sort(key=lambda pair: pair[1], reverse=True)
    return compound_stat[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    data = read_csv_as_nested_dict(info["masterfile"],
                                   info["playerid"],
                                   info["separator"],
                                   info["quote"])
    new_data = []
    for id_stat in top_ids_and_stats:
        new_data.append("{:.3f} --- {} {}".format(id_stat[1], 
                                                  data[id_stat[0]][info["firstname"]], 
                                                  data[id_stat[0]][info["lastname"]]))
    return new_data
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    statistics = read_csv_as_list_dict(info["battingfile"], 
                                       info["separator"], 
                                       info["quote"])
    stats = filter_by_year(statistics, year, info["yearid"])
    top_ids = top_player_ids(info, stats, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics:
        if row[playerid] in data:
            for item in fields:
                data[row[playerid]][item] += float(row[item])
        else:
            data[row[playerid]] = {}
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    statistics = read_csv_as_list_dict(info["battingfile"], 
                                       info["separator"], 
                                       info["quote"])
    aggregate_stats = aggregate_by_player_id(statistics, 
                                             info["playerid"], 
                                             info["battingfields"])
    stats = top_player_ids(info, aggregate_stats.values(), 
                           formula, numplayers)
    return lookup_player_names(info, stats)
